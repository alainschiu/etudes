            ...
            {itemsToShow.map(item=>{
              ...
              return (<div key={`${session.id}-${item.id}`} data-today-item={item.id} style={{borderBottom:`1px solid ${LINE}`}}>
                ...
                {expanded&&(<div className="px-6 py-5 space-y-4" style={{background:SURFACE,borderTop:`1px solid ${LINE}`}}>
                  ...
                  {item.type==='piece'&&<SpotsBlock item={item} itemTimes={itemTimes} activeItemId={activeItemId} activeSpotId={activeSpotId} startItem={(id,sid)=>startItem(id,sid,session.id)} stopItem={stopItem} addSpot={addSpot} updateSpot={updateSpot} deleteSpot={deleteSpot} editSpotTime={editSpotTime} dayClosed={dayClosed} setConfirmModal={p.setConfirmModal}/>}
