...
export default function TodayView(p){
  ...
  const getAvailableItems=(s)=>{const t=new Set(s.itemIds||[]);return items.filter(i=>i.type===s.type&&!t.has(i.id));};
  ...
  return (
    <div className="max-w-3xl mx-auto px-12 py-14">
            ...
            <div className="group flex items-center justify-between py-3 px-2 cursor-pointer" style={{borderBottom:`1px solid ${LINE}`}} onClick={()=>toggleCollapsed(session.id)}>
              ...
              <div className="flex items-center gap-2 shrink-0" onClick={e=>e.stopPropagation()}>
                <span className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5">
                  ...
                  <button onClick={()=>addItem(type)} style={{color:FAINT}} title="New repertoire item"><Plus className="w-3.5 h-3.5" strokeWidth={1.25}/></button>
                </span>
              ...
              return (<div key={`${session.id}-${item.id}`} style={{borderBottom:`1px solid ${LINE}`}}>
                <div onClick={()=>{if(!isEditingTime)setExpandedItemId(expanded?null:item.id);}} className="group py-2.5 px-2 flex items-center gap-4 cursor-pointer" style={{background:isActiveAny?IKB_SOFT:'transparent'}}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-baseline gap-2 min-w-0 flex-wrap">
                      ...
                      {item.type==='piece'&&<button onClick={e=>{e.stopPropagation();setPdfDrawerItemId(item.id);}} className="inline-flex items-center shrink-0" style={{color:hasPdf?TEXT:FAINT,padding:'0 2px'}} title={hasPdf?`${item.pdfs.length} score${item.pdfs.length===1?'':'s'}`:'No score yet'}><FileText className="w-3 h-3" strokeWidth={1.25}/></button>}
                ...
                {expanded&&(<div className="px-6 py-5 space-y-4" style={{background:SURFACE,borderTop:`1px solid ${LINE}`}}>
                  <div className="grid grid-cols-2 gap-4">
                    <div><div className="uppercase mb-1.5" style={{color:FAINT,fontSize:'10px',letterSpacing:'0.25em'}}>Title</div><input type="text" value={item.title} onChange={e=>updateItem(item.id,{title:e.target.value})} className="w-full px-0 py-1 text-sm focus:outline-none" style={{background:'transparent',color:TEXT,borderBottom:`1px solid ${LINE_MED}`}}/></div>
                    <div><div className="uppercase mb-1.5" style={{color:FAINT,fontSize:'10px',letterSpacing:'0.25em'}}>Composer</div><input type="text" value={item.composer||''} onChange={e=>updateItem(item.id,{composer:e.target.value})} placeholder="—" className="w-full px-0 py-1 text-sm italic focus:outline-none" style={{background:'transparent',color:TEXT,borderBottom:`1px solid ${LINE_MED}`,fontFamily:serif}}/></div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs flex-wrap">
                    <input type="text" placeholder="add tag + enter" onKeyDown={e=>{if(e.key==='Enter'&&e.target.value.trim()){const tag=e.target.value.trim().replace(/^#/,'');if(!item.tags.includes(tag))updateItem(item.id,{tags:[...item.tags,tag]});e.target.value='';}}} className="px-2 py-1 text-xs focus:outline-none" style={{background:SURFACE2,color:TEXT,border:`1px solid ${LINE_MED}`}}/>
                    {item.tags.map(t=>(<span key={t} className="px-2 py-1 flex items-center gap-1" style={{background:SURFACE2,border:`1px solid ${LINE}`,fontSize:'9px',letterSpacing:'0.18em'}}>{t.toUpperCase()}<button onClick={()=>updateItem(item.id,{tags:item.tags.filter(x=>x!==t)})}><X className="w-2.5 h-2.5" strokeWidth={1.25}/></button></span>))}
      ...
      <div className="mt-16"><div className="uppercase mb-3" style={{color:FAINT,fontSize:'10px',letterSpacing:'0.32em'}}>Reflection</div><h3 className="text-4xl mb-6 leading-none" style={{fontFamily:serif,fontStyle:'italic',fontWeight:300,letterSpacing:'-0.015em'}}>Journal du jour</h3><textarea ref={reflectionRef} value={dailyReflection} onChange={e=>setDailyReflection(e.target.value)} placeholder="How did today feel? What surprised you?" className="w-full h-44 p-6 resize-none focus:outline-none" style={{background:SURFACE,color:TEXT,border:`1px solid ${LINE}`,fontFamily:serif,fontSize:'16px',lineHeight:1.75}}/></div>
    </div>
  );
}
