...
// Mirrors the wiki-link <a> handling from MarkdownComponents in shared.jsx;
// the visual styling diverges so a shared helper is deferred.
const logMd=({onWikiLinkClick,items,history,programs,notes}={})=>({
  ...
  a:({href,children})=>{
    ...
    if(isWiki){
      ...
      const resolved=onWikiLinkClick?resolveWikiLink(wikiRaw,items||[],history||[],programs||[],notes||[]):null;
      const suppress=(e)=>{e.preventDefault();e.stopPropagation();};
      const handleClick=(e)=>{suppress(e);if(resolved&&onWikiLinkClick)onWikiLinkClick(wikiRaw);};
      const wikilinkStyle=resolved
      ...
      return (<a href={href} onClick={handleClick} onTouchStart={suppress} style={wikilinkStyle} title={resolved?undefined:'no match'}>{children}</a>);
    }
    const handleClick=(e)=>{e.preventDefault();if(href)window.open(href,'_blank','noopener,noreferrer');};
...
export function LogDrawer({entry,dayData,items,history,programs,recordingMeta,freeNotes,onWikiLinkClick,onClose,deleteRecording}){
  ...
  const wikiProps={onWikiLinkClick,items,history,programs,notes:freeNotes};
  return (<div className="fixed inset-0 z-50 flex"><div className="flex-1" style={{background:'rgba(0,0,0,0.7)',backdropFilter:'blur(4px)'}} onClick={onClose}/><div className="w-full max-w-2xl flex flex-col overflow-hidden" style={{background:BG,borderLeft:`1px solid ${LINE_STR}`}}>{kind==='day'&&<DayLogContent dayData={dayData} items={items} recordingMeta={recordingMeta} deleteRecording={deleteRecording} onClose={onClose} wikiProps={wikiProps}/>}{kind==='week'&&<WeekLogContent entry={entry} onClose={onClose} wikiProps={wikiProps}/>}{kind==='month'&&<MonthLogContent entry={entry} onClose={onClose} wikiProps={wikiProps}/>}{rn.length>0&&<NearbyNotes notes={rn}/>}</div></div>);
}

function DayLogContent({dayData,items,recordingMeta,deleteRecording,onClose,wikiProps}){
