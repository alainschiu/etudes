...
import {BG, SURFACE, SURFACE2, TEXT, MUTED, FAINT, DIM, LINE, LINE_MED, LINE_STR, IKB, IKB_SOFT, WARM, serif, sans, mono, LINK} from '../constants/theme.js';
...
import {Waveform, MarkdownComponents, preprocessWikiLinks, wikiUrlTransform} from '../components/shared.jsx';

export function LogsView(p){
  const {isMobile}=useViewport();
  const {history,items,openLogEntry,recordingMeta,settings,freeNotes,onWikiLinkClick,wikiCompletionData}=p;const dailyTarget=settings?.dailyTarget||90;
...
// Render markdown for a log read surface. Pre-processes [[wiki]] links so
// the shared anchor renderer in MarkdownComponents picks them up; the link
// is then resolved against history/items/programs/notes from completionData
// and routed through onWikiLinkClick (top-level handler in App.jsx).
function LogMarkdown({source,variant='log',onWikiLinkClick,completionData}){
  if(!source)return null;
  return(
    <ReactMarkdown
      remarkPlugins={[remarkGfm]}
      urlTransform={wikiUrlTransform}
      components={MarkdownComponents({variant,onWikiLinkClick,completionData})}
    >
...
export function LogDrawer({entry,dayData,items,recordingMeta,freeNotes,onClose,deleteRecording,onWikiLinkClick,wikiCompletionData}){
  ...
  return (<div className="fixed inset-0 z-50 flex"><div className="flex-1" style={{background:'rgba(0,0,0,0.7)',backdropFilter:'blur(4px)'}} onClick={onClose}/><div className="w-full max-w-2xl flex flex-col overflow-hidden" style={{background:BG,borderLeft:`1px solid ${LINE_STR}`}}>{kind==='day'&&<DayLogContent dayData={dayData} items={items} recordingMeta={recordingMeta} deleteRecording={deleteRecording} onClose={onClose} onWikiLinkClick={onWikiLinkClick} wikiCompletionData={wikiCompletionData}/>}{kind==='week'&&<WeekLogContent entry={entry} onClose={onClose} onWikiLinkClick={onWikiLinkClick} wikiCompletionData={wikiCompletionData}/>}{kind==='month'&&<MonthLogContent entry={entry} onClose={onClose} onWikiLinkClick={onWikiLinkClick} wikiCompletionData={wikiCompletionData}/>}{rn.length>0&&<NearbyNotes notes={rn}/>}</div></div>);
}

function DayLogContent({dayData,items,recordingMeta,deleteRecording,onClose,onWikiLinkClick,wikiCompletionData}){
