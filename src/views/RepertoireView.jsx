        ...
        <div style={{padding:'0 0 16px'}}>
          ...
          {(item.spots||[]).map((spot,idx)=>(
            <div key={spot.id} style={{borderBottom:`1px solid ${LINE}`}}>
              <SpotEditor spot={spot} itemId={item.id} itemTimes={itemTimes} isActive={activeItemId===item.id&&activeSpotId===spot.id} onStart={()=>startItem(item.id,spot.id)} onStop={stopItem} onUpdate={patch=>updateSpot(item.id,spot.id,patch)} onDelete={()=>deleteSpot(item.id,spot.id)} onMoveUp={()=>{}} onMoveDown={()=>{}} canMoveUp={idx>0} canMoveDown={idx<(item.spots||[]).length-1} onEditTime={v=>editSpotTime(item.id,spot.id,v)} dayClosed={dayClosed} itemPdfs={item.pdfs||[]} defaultPdfId={item.defaultPdfId} onWikiLinkClick={onWikiLinkClick} wikiCompletionData={wikiCompletionData}/>
            </div>
          ))}
