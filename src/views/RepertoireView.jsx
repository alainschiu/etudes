...
function FilterDropdown({label,value,options,onChange}){const [open,setOpen]=useState(false);const current=options.find(o=>o.value===value);return (<div className="relative"><button onClick={()=>setOpen(v=>!v)} className="flex items-baseline gap-2 px-3 py-1.5 italic" style={{background:value?IKB_SOFT:'transparent',border:`1px solid ${value?IKB:LINE_MED}`,color:value?TEXT:MUTED,fontFamily:serif,fontSize:'13px',fontWeight:300}}><span className="uppercase not-italic" style={{fontFamily:sans,fontSize:'9px',letterSpacing:'0.22em',color:FAINT}}>{label}</span><span>{current?.label||'All'}</span><ChevronDown className="w-3 h-3 not-italic" strokeWidth={1.25}/></button>{open&&(<><div className="fixed inset-0 z-20" onClick={()=>setOpen(false)}/><div className="absolute top-full mt-1 z-30 min-w-40 max-h-64 overflow-auto etudes-scroll" style={{background:SURFACE,border:`1px solid ${LINE_STR}`,boxShadow:'0 4px 20px rgba(0,0,0,0.5)'}}>{options.map(o=>(<button key={o.value||'_all'} onClick={()=>{onChange(o.value);setOpen(false);}} className="w-full text-left px-3 py-2" style={{background:value===o.value?IKB_SOFT:'transparent',color:value===o.value?TEXT:MUTED,borderBottom:`1px solid ${LINE}`,fontFamily:serif,fontSize:'13px',fontStyle:'italic',fontWeight:300}}>{o.label}</button>))}</div></>)}</div>);}
...
function SortDropdown({value,onChange}){
  ...
  return (<div className="relative"><button onClick={()=>setOpen(v=>!v)} className="flex items-baseline gap-2 px-3 py-1.5 italic" style={{background:value?IKB_SOFT:'transparent',border:`1px solid ${value?IKB:LINE_MED}`,color:value?TEXT:MUTED,fontFamily:serif,fontSize:'13px',fontWeight:300}}><span className="uppercase not-italic" style={{fontFamily:sans,fontSize:'9px',letterSpacing:'0.22em',color:FAINT}}>Sort</span><span>{current?.label||'Default'}</span><ChevronDown className="w-3 h-3 not-italic" strokeWidth={1.25}/></button>{open&&(<><div className="fixed inset-0 z-20" onClick={()=>setOpen(false)}/><div className="absolute top-full mt-1 z-30 min-w-40" style={{background:SURFACE,border:`1px solid ${LINE_STR}`,boxShadow:'0 4px 20px rgba(0,0,0,0.5)'}}>{opts.map(o=>(<button key={o.value||'_def'} onClick={()=>{onChange(o.value);setOpen(false);}} className="w-full text-left px-3 py-2" style={{background:value===o.value?IKB_SOFT:'transparent',color:value===o.value?TEXT:MUTED,borderBottom:`1px solid ${LINE}`,fontFamily:serif,fontSize:'13px',fontStyle:'italic',fontWeight:300}}>{o.label}</button>))}</div></>)}</div>);}
...
export default function RepertoireView(p){
      ...
      <div onClick={()=>setExpandedId(expanded?null:i.id)} className="py-3 px-2 flex items-start gap-3 cursor-pointer" style={{background:expanded?SURFACE:'transparent'}}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-3 flex-wrap">
            <span style={{fontWeight:300,fontSize:'14px'}}>{displayTitle(i)}</span>
            {i.catalog&&<span className="tabular-nums" style={{color:FAINT,fontSize:'11px',fontFamily:serif,fontStyle:'italic'}}>{i.catalog}</span>}
        ...
        {expanded?<ChevronUp className="w-3.5 h-3.5 shrink-0 mt-1" strokeWidth={1.25} style={{color:FAINT}}/>:<ChevronDown className="w-3.5 h-3.5 shrink-0 mt-1" strokeWidth={1.25} style={{color:FAINT}}/>}
      </div>
        ...
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-8">
            <div className="mb-5">
              ...
              {showBpmTarget&&<EditorRow label="Tempo target" icon={<TrendingUp className="w-3 h-3" strokeWidth={1.25}/>}><input type="number" min="40" max="300" value={i.bpmTarget??''} onChange={e=>{const v=e.target.value;const n=parseInt(v,10);updateItem(i.id,{bpmTarget:Number.isFinite(n)&&n>0?n:null});}} placeholder="— bpm" style={eInM}/></EditorRow>}
          ...
          <div className="col-span-4 space-y-5 min-w-0">
            ...
            <div><div className="uppercase mb-2" style={{color:FAINT,fontSize:'10px',letterSpacing:'0.28em'}}>Last practiced</div><div style={{fontFamily:serif,fontStyle:'italic',fontSize:'18px',fontWeight:300,color:rel?TEXT:FAINT}}>{rel||'—'}</div></div>
            ...
            <div className="pt-4 space-y-2" style={{borderTop:`1px solid ${LINE}`}}>
              <button onClick={()=>isActiveAny?stopItem():startItem(i.id)} disabled={dayClosed&&!isActiveAny} className="w-full uppercase px-3 py-2.5 flex items-center justify-center gap-2" style={isActiveWhole?{background:IKB,color:TEXT,border:`1px solid ${IKB}`,boxShadow:`0 0 15px ${IKB}60`,fontSize:'10px',letterSpacing:'0.22em'}:{background:'transparent',color:dayClosed?FAINT:TEXT,border:`1px solid ${LINE_STR}`,fontSize:'10px',letterSpacing:'0.22em',cursor:(dayClosed&&!isActiveAny)?'not-allowed':'pointer'}}>{isActiveAny?<><Pause className="w-3 h-3" strokeWidth={1.25}/> Pause</>:<><Play className="w-3 h-3" strokeWidth={1.25}/> {dayClosed?'Day closed':'Practice whole piece'}</>}</button>
              <button onClick={()=>setPdfDrawerItemId(i.id)} className="w-full uppercase px-3 py-2.5 flex items-center justify-center gap-2" style={{color:TEXT,border:`1px solid ${LINE_STR}`,fontSize:'10px',letterSpacing:'0.22em'}}><FileText className="w-3 h-3" strokeWidth={1.25}/> {(i.pdfs||[]).length>0?`Scores (${i.pdfs.length})`:'Add score'}</button>
              {(i.type==='piece'||i.type==='play')&&<button onClick={()=>updateItem(i.id,{type:i.type==='piece'?'play':'piece'})} className="w-full uppercase px-3 py-2.5 flex items-center justify-center gap-2" style={{color:MUTED,border:`1px solid ${LINE_MED}`,fontSize:'10px',letterSpacing:'0.22em'}}>Move to {i.type==='piece'?'Play':'Pieces'}</button>}
              <button onClick={()=>{deleteItem(i.id);setExpandedId(null);}} className="w-full uppercase px-3 py-2.5 flex items-center justify-center gap-2" style={{color:MUTED,border:`1px solid ${LINE}`,fontSize:'10px',letterSpacing:'0.22em'}}><Trash2 className="w-3 h-3" strokeWidth={1.25}/> Delete</button>
            </div>
          </div>
        </div>
      </div>)}
  ...
  return (<div className="flex max-w-6xl mx-auto">
    ...
    {sidebarOpen&&(<aside className="w-52 shrink-0 px-6 py-14 space-y-8" style={{borderRight:`1px solid ${LINE}`}}><div className="flex items-center justify-end -mt-4"><button onClick={()=>setSidebarOpen(false)} style={{color:DIM}}><ChevronDown className="w-3 h-3" strokeWidth={1.25}/></button></div><SidebarFacet title="Composers" icon={<Users className="w-3 h-3" strokeWidth={1.25}/>} open={composerOpen} setOpen={setComposerOpen} entries={allComposers} activeValue={filterComposer} onSelect={setFilterComposer} emptyText="No composers yet."/><SidebarFacet title="Instruments" icon={<Guitar className="w-3 h-3" strokeWidth={1.25}/>} open={instrumentOpen} setOpen={setInstrumentOpen} entries={allInstruments} activeValue={filterInstrument} onSelect={setFilterInstrument} emptyText="No instruments set."/></aside>)}
    <div className={`flex-1 min-w-0 ${sidebarOpen?'px-10':'px-12'} py-14`}>
      <div className="flex items-center gap-2 mb-3">{!sidebarOpen&&<button onClick={()=>setSidebarOpen(true)} className="uppercase flex items-center gap-1.5 px-2 py-1" style={{color:MUTED,border:`1px solid ${LINE_MED}`,fontSize:'9px',letterSpacing:'0.22em'}}><Users className="w-3 h-3" strokeWidth={1.25}/> Facets</button>}</div>
      <DisplayHeader eyebrow="Library" title="Repertoire"/>
      <div className="mb-6 p-5" style={{background:SURFACE,border:`1px solid ${LINE}`}}><div className="uppercase mb-3" style={{color:FAINT,fontSize:'10px',letterSpacing:'0.28em'}}>Add new</div><div className="flex gap-2 flex-wrap">{TYPES.map(t=>(<button key={t} onClick={()=>handleAdd(t)} className="uppercase px-3 py-2 flex items-center gap-1.5" style={{color:TEXT,border:`1px solid ${LINE_STR}`,fontSize:'10px',letterSpacing:'0.22em'}}><Plus className="w-3 h-3" strokeWidth={1.25}/><span style={{color:DIM}}>{SECTION_CONFIG[t].roman}.</span> {SECTION_CONFIG[t].label}</button>))}</div></div>
      <div className="flex items-center gap-2 mb-5 py-3" style={{borderTop:`1px solid ${LINE_STR}`,borderBottom:`1px solid ${LINE}`}}>
        ...
        <FilterDropdown label="Style" value={filterStyle} options={[{value:'',label:'All'},...allTags.map(t=>({value:t,label:t}))]} onChange={setFilterStyle}/>
        {(filterComposer||filterInstrument)&&<span className="italic" style={{color:FAINT,fontFamily:serif,fontSize:'11px'}}>{filterComposer&&<>composer: <span style={{color:TEXT}}>{filterComposer}</span></>}{filterComposer&&filterInstrument&&<span style={{margin:'0 6px'}}>·</span>}{filterInstrument&&<>instrument: <span style={{color:TEXT}}>{filterInstrument}</span></>}</span>}
        {hasFilters&&<button onClick={clearFilters} className="uppercase" style={{color:MUTED,fontSize:'9px',letterSpacing:'0.22em'}}>Clear</button>}
        <div className="ml-auto flex items-center gap-2">
          <div style={{width:'1px',height:'16px',background:LINE_STR}}/>
          <SortDropdown value={sortBy} onChange={setSortBy}/>
          <button onClick={()=>setGroupByCollection(v=>!v)} className="uppercase flex items-center gap-1.5 px-3 py-1.5 shrink-0" style={{color:groupByCollection?TEXT:MUTED,border:`1px solid ${groupByCollection?IKB:LINE_MED}`,background:groupByCollection?IKB_SOFT:'transparent',fontSize:'10px',letterSpacing:'0.22em'}}><Layers className="w-3 h-3" strokeWidth={1.25}/> Group</button>
          <div style={{width:'1px',height:'16px',background:LINE_STR}}/>
          <Search className="w-3.5 h-3.5 shrink-0" strokeWidth={1.25} style={{color:FAINT}}/>
          <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search…" className="text-sm focus:outline-none" style={{background:'transparent',color:TEXT,width:'140px'}}/>
        </div>
      </div>
      <div style={{borderTop:`1px solid ${LINE_STR}`}}>
        ...
        {items.length>0&&sorted.length===0&&(<div className="py-12 text-center"><div className="italic mb-3" style={{color:FAINT,fontFamily:serif,fontSize:'15px'}}>Nothing matches these filters.</div><button onClick={clearFilters} className="uppercase px-4 py-2" style={{color:TEXT,border:`1px solid ${LINE_STR}`,fontSize:'10px',letterSpacing:'0.22em'}}>Clear all filters</button></div>)}
      </div>
    </div>
  </div>);
}
