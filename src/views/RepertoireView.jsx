...
import {getItemTime, getSpotTime, displayTitle, formatByline, normalizeComposerKey, nextPerformance, mkSpotId, resolveScoreLinkPage} from '../lib/items.js';
...
function SpotEditor({spot,itemId,itemTimes,isActive,onStart,onStop,onUpdate,onDelete,onMoveUp,onMoveDown,canMoveUp,canMoveDown,onEditTime,dayClosed,itemPdfs=[],defaultPdfId=null,onWikiLinkClick,wikiCompletionData,requestScoreView}){const [label,setLabel]=useState(spot.label);const [bpm,setBpm]=useState(spot.bpmTarget||'');const [editingTime,setEditingTime]=useState(false);const [editingPage,setEditingPage]=useState(false);const [pageVal,setPageVal]=useState('');useEffect(()=>{setLabel(spot.label);setBpm(spot.bpmTarget||'');},[spot.label,spot.bpmTarget]);const time=getSpotTime(itemTimes,itemId,spot.id);const hbl=(spot.bpmLog||[]).length>=2;const cL=()=>{if(label.trim()&&label.trim()!==spot.label)onUpdate({label:label.trim()});else setLabel(spot.label);};const cB=()=>{const n=parseInt(bpm,10);const v=Number.isFinite(n)&&n>0?n:null;if(v!==spot.bpmTarget)onUpdate({bpmTarget:v});};return (<div className="group px-3 py-3 space-y-2" style={{background:isActive?IKB_SOFT:SURFACE2,borderLeft:isActive?`2px solid ${IKB}`:`2px solid transparent`,minWidth:0}}><div className="flex items-center gap-2 min-w-0"><button onClick={()=>isActive?onStop():onStart()} disabled={dayClosed&&!isActive} className="shrink-0" style={{color:isActive?IKB:(dayClosed?FAINT:TEXT),filter:isActive?`drop-shadow(0 0 4px ${IKB})`:'none',cursor:(dayClosed&&!isActive)?'not-allowed':'pointer'}}>{isActive?<Pause className="w-4 h-4" strokeWidth={1.25} fill="currentColor"/>:<Play className="w-4 h-4" strokeWidth={1.25} fill="currentColor"/>}</button><Crosshair className="w-3 h-3 shrink-0" strokeWidth={1.25} style={{color:isActive?IKB:FAINT}}/><input value={label==='New spot'?'':label} onFocus={selectOnFocus} onChange={e=>setLabel(e.target.value)} onBlur={cL} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();}} placeholder="New spot" className="flex-1 min-w-0 focus:outline-none pb-0.5" style={{background:'transparent',color:TEXT,borderBottom:`1px solid ${LINE_MED}`,fontFamily:serif,fontSize:'14px',fontWeight:300}}/><div className="flex items-center gap-1 shrink-0"><span className="uppercase" style={{color:FAINT,fontSize:'9px',letterSpacing:'0.22em'}}>♩</span><input value={bpm} onFocus={selectOnFocus} onChange={e=>setBpm(e.target.value.replace(/[^0-9]/g,''))} onBlur={cB} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();}} placeholder="—" className="w-12 text-right font-mono tabular-nums focus:outline-none pb-0.5" style={{background:'transparent',color:TEXT,borderBottom:`1px solid ${LINE_MED}`,fontSize:'11px'}}/></div>{editingTime?(<ItemTimeEditor seconds={time} onCommit={(v)=>{onEditTime(v);setEditingTime(false);}} onCancel={()=>setEditingTime(false)} small/>):(<><span className="font-mono tabular-nums shrink-0 w-14 text-right" style={{color:time>0?MUTED:FAINT,fontSize:'11px'}}>{time>0?fmtSpotTime(time):'—'}</span>{!dayClosed&&<button onClick={()=>setEditingTime(true)} className="target-hover-reveal shrink-0" style={{color:FAINT}}><Pencil className="w-3 h-3" strokeWidth={1.25}/></button>}</>)}{itemPdfs.length>0&&(()=>{const resolvedPage=resolveScoreLinkPage(spot.scoreLink,itemPdfs);const setPageLink=(pg)=>{const attId=spot.scoreLink?.attId||defaultPdfId||itemPdfs[0]?.id||null;onUpdate({scoreLink:(attId&&pg)?{attId,bookmarkId:null,page:pg}:null});};return editingPage?(<input autoFocus type="text" inputMode="numeric" placeholder="page" value={pageVal} onChange={e=>setPageVal(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){const n=parseInt(pageVal,10);setPageLink(Number.isFinite(n)&&n>0?n:null);setEditingPage(false);}else if(e.key==='Escape')setEditingPage(false);}} onBlur={()=>{const n=parseInt(pageVal,10);setPageLink(Number.isFinite(n)&&n>0?n:null);setEditingPage(false);}} className="shrink-0" style={{width:'48px',background:'transparent',color:TEXT,border:`1px solid ${LINE_MED}`,fontSize:'10px',padding:'1px 4px',outline:'none',fontFamily:mono,textAlign:'center'}}/>):resolvedPage?(<span className="shrink-0 flex items-center gap-0.5"><button onClick={requestScoreView?()=>requestScoreView({itemId,...spot.scoreLink}):()=>{setPageVal(String(resolvedPage));setEditingPage(true);}} className="shrink-0 flex items-center gap-0.5" title={requestScoreView?`Jump to score p.${resolvedPage}`:`Score p.${resolvedPage}`} style={{position:'relative',color:IKB,fontSize:'10px',fontFamily:mono,background:'transparent',border:'none',cursor:'pointer',padding:'1px 3px',lineHeight:1}}><span aria-hidden="true" style={{position:'absolute',top:'-10px',bottom:'-10px',left:'-6px',right:'-4px'}}/><FileText className="w-3 h-3" strokeWidth={1.5}/>{resolvedPage}</button><button onClick={()=>{setPageVal(String(resolvedPage));setEditingPage(true);}} className="target-hover-reveal" title="Edit score link" style={{color:FAINT,background:'transparent',border:'none',cursor:'pointer',padding:'1px 2px',lineHeight:1}}><Pencil className="w-3 h-3" strokeWidth={1.5}/></button></span>):(<button onClick={()=>{setPageVal('');setEditingPage(true);}} className="target-hover-reveal shrink-0" title="Link to a PDF page" style={{color:FAINT,background:'transparent',border:'none',cursor:'pointer',padding:'1px 3px',lineHeight:1}}><FileText className="w-3 h-3" strokeWidth={1.5}/></button>);})()}<button onClick={onMoveUp} disabled={!canMoveUp} className="target-hover-reveal shrink-0" style={{color:canMoveUp?FAINT:DIM}}><ArrowUp className="w-3 h-3" strokeWidth={1.25}/></button><button onClick={onMoveDown} disabled={!canMoveDown} className="target-hover-reveal shrink-0" style={{color:canMoveDown?FAINT:DIM}}><ArrowDown className="w-3 h-3" strokeWidth={1.25}/></button><button onClick={onDelete} className="target-hover-reveal shrink-0" style={{color:FAINT}}><Trash2 className="w-3 h-3" strokeWidth={1.25}/></button></div>{hbl&&(<div className="pl-9 pt-1 flex items-center gap-3"><span className="uppercase shrink-0" style={{color:FAINT,fontSize:'9px',letterSpacing:'0.22em'}}>Tempo</span><BpmSparkline log={spot.bpmLog} target={spot.bpmTarget} compact/><span className="tabular-nums shrink-0" style={{color:DIM,fontSize:'9px',letterSpacing:'0.22em'}}>{spot.bpmLog.length} log{spot.bpmLog.length===1?'':'s'}</span></div>)}<MarkdownField value={spot.note||''} onChange={v=>onUpdate({note:v})} placeholder="Persistent note…" minHeight={48} style={{background:BG,border:`1px solid ${LINE}`}} onWikiLinkClick={onWikiLinkClick} completionData={wikiCompletionData}/>{itemPdfs.length>0&&(()=>{const allBm=itemPdfs.flatMap(att=>(att.bookmarks||[]).map(bm=>({attId:att.id,bm})));const cur=spot.scoreLink?.bookmarkId?allBm.find(x=>x.attId===spot.scoreLink.attId&&x.bm.id===spot.scoreLink.bookmarkId):null;const val=cur?`${cur.attId}::${cur.bm.id}`:'';return(<div className="pl-0 flex items-center gap-2"><span className="uppercase shrink-0" style={{color:FAINT,fontSize:'9px',letterSpacing:'0.22em'}}>→ bookmark</span><select value={val} onChange={e=>{const v=e.target.value;if(!v){onUpdate({scoreLink:null});return;}const[attId,bmId]=v.split('::');onUpdate({scoreLink:{attId,bookmarkId:bmId,page:null}});}} className="focus:outline-none" style={{background:BG,color:val?TEXT:FAINT,border:`1px solid ${LINE_MED}`,fontSize:'11px',padding:'1px 4px',cursor:'pointer'}}><option value="">— none —</option>{allBm.map(({attId,bm})=>(<option key={`${attId}::${bm.id}`} value={`${attId}::${bm.id}`}>{bm.name} · p.{bm.page}</option>))}</select></div>)})()}</div>);}
...
export default function RepertoireView(p){
  ...
  useEffect(()=>{
    ...
    const handler=(e)=>{
      ...
      // Without this, confirming a destructive ConfirmModal that originated
      // from inside the expanded card (e.g. delete recording, delete piece)
        ...
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-8">
            <div className="mb-5">
              ...
              <EditorRow label="Composer"><DebouncedField list="rep-composer-list" suggestions={uniqueComposerNames} type="text" value={i.composer||''} onChange={v=>updateItem(i.id,{composer:v})} placeholder="Composer" style={eInI}/></EditorRow>
              ...
              <EditorRow label="Instrument" icon={<Guitar className="w-3 h-3" strokeWidth={1.25}/>}><DebouncedField list="rep-instrument-list" suggestions={uniqueInstrumentNames} type="text" value={i.instrument||''} onChange={v=>updateItem(i.id,{instrument:v})} placeholder="Instrument" style={{...eIn,fontFamily:serif,fontSize:'13px'}}/></EditorRow>
        ...
        <div style={{padding:'0 0 16px'}}>
          ...
          {(item.spots||[]).map((spot,idx)=>(
            <div key={spot.id} style={{borderBottom:`1px solid ${LINE}`}}>
              <SpotEditor spot={spot} itemId={item.id} itemTimes={itemTimes} isActive={activeItemId===item.id&&activeSpotId===spot.id} onStart={()=>startItem(item.id,spot.id)} onStop={stopItem} onUpdate={patch=>updateSpot(item.id,spot.id,patch)} onDelete={()=>deleteSpot(item.id,spot.id)} onMoveUp={()=>{}} onMoveDown={()=>{}} canMoveUp={idx>0} canMoveDown={idx<(item.spots||[]).length-1} onEditTime={v=>editSpotTime(item.id,spot.id,v)} dayClosed={dayClosed} itemPdfs={item.pdfs||[]} defaultPdfId={item.defaultPdfId} onWikiLinkClick={onWikiLinkClick} wikiCompletionData={wikiCompletionData} requestScoreView={requestScoreView}/>
            </div>
          ))}
