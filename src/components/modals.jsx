...
import {X, Download, Archive, Upload as UploadIcon} from 'lucide-react';
...
export function SettingsModal({settings,setSettings,storageMode,onExportMd,onExportTxt,onExportJson,onImportClick,onClose}){
  ...
  return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(0,0,0,0.7)',backdropFilter:'blur(8px)'}} onClick={onClose}><div className="max-w-md w-full max-h-screen overflow-auto etudes-scroll" style={{background:BG,border:`1px solid ${LINE_STR}`}} onClick={e=>e.stopPropagation()}>
    ...
    <div className="flex px-8" style={{borderBottom:`1px solid ${LINE}`}}>
      {[{id:'settings',label:'Settings'},{id:'export',label:'Export'},{id:'shortcuts',label:'Shortcuts'},{id:'about',label:'About'}].map(t=>(
    ...
    <div className="px-8 py-5" style={{borderTop:`1px solid ${LINE}`}}><button onClick={onClose} className="w-full py-3 uppercase" style={{background:IKB,color:TEXT,boxShadow:`0 0 20px ${IKB}40`,fontSize:'10px',letterSpacing:'0.32em'}}>Done</button></div>
  </div></div>);
}
