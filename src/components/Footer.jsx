...
export default function Footer({isMobile,metronome,setMetronome,metroExpanded,setMetroExpanded,drone,setDrone,droneExpanded,setDroneExpanded,toggleDrone,currentBeat,currentSub,activeItemId,activeSpotId,activeItem,activeSpot,activeIsWarmup,sectionTimes,totalToday,effectiveTotalToday,warmupTimeToday,restToday,isResting,toggleRest,itemTimes,fmt,fmtMin,stopItem,handleTap,isRecording,startRecording,stopRecording,logTempo,quickNoteOpen,setQuickNoteOpen,addQuickNote,dayClosed,dayJustRolled,recExpanded,setRecExpanded,recordingMeta,deleteRecording,todayKey,startPieceRecording,stopPieceRecording,pieceRecordingItemId,pieceRecordingMeta,attachDailyToPiece,todaySessions,items,settings,handleStartRecording}){
  ...
  return (<footer className={isMobile?'':'shrink-0'} style={{borderTop:`1px solid ${LINE_MED}`,background:BG,...(isMobile&&{position:'fixed',bottom:0,left:0,right:0,zIndex:Z_FOOTER})}}>
    ...
    <div className="h-16 flex items-center px-10 gap-6">
      ...
      {/* ── Middle cluster: locked widths, shared 32 px height, idle border = LINE_MED ── */}
      <div className="flex items-center gap-2 shrink-0">
        <Tooltip shortcut="R"><button onClick={toggleRest} disabled={dayClosed&&!isResting} style={{height:32,padding:'0 12px',display:'inline-flex',alignItems:'center',gap:8,border:`1px solid ${isResting?IKB:LINE_MED}`,color:isResting?IKB:((dayClosed&&!isResting)?FAINT:MUTED),background:isResting?IKB_SOFT:'transparent',cursor:(dayClosed&&!isResting)?'not-allowed':'pointer'}}><Coffee className="w-3 h-3" strokeWidth={1.25}/><span className="uppercase tabular-nums" style={{fontSize:'10px',letterSpacing:'0.22em',display:'inline-block',minWidth:44,textAlign:'left'}}>{isResting?`${Math.floor(restToday/60)}′ rest`:'Rest'}</span></button></Tooltip>
        <button onClick={handleRecordClick} disabled={dayClosed&&!anyRecording} style={{height:32,padding:'0 12px',display:'inline-flex',alignItems:'center',gap:8,border:`1px solid ${anyRecording?REC:(recExpanded&&todayRec?IKB:LINE_MED)}`,color:anyRecording?REC:((dayClosed&&!anyRecording)?FAINT:MUTED),background:anyRecording?'rgba(169,50,38,0.08)':(recExpanded&&todayRec?IKB_SOFT:'transparent'),cursor:(dayClosed&&!anyRecording)?'not-allowed':'pointer'}}>{anyRecording?<><Square className="w-3 h-3" strokeWidth={1.25} fill="currentColor"/><span className="uppercase animate-pulse" style={{fontSize:'10px',letterSpacing:'0.22em',display:'inline-block',minWidth:60,textAlign:'left'}}>{isActivePieceRec?'Piece':'REC'}</span></>:<><Mic className="w-3 h-3" strokeWidth={1.25}/><span className="uppercase" style={{fontSize:'10px',letterSpacing:'0.22em',display:'inline-block',minWidth:60,textAlign:'left'}}>{activeItemId&&startPieceRecording?'Rec piece':'Record'}</span></>}</button>
        ...
        {todayRec&&!isRecording&&<button onClick={()=>setRecExpanded(x=>!x)} title="Play today's recording" style={{height:32,width:32,display:'inline-flex',alignItems:'center',justifyContent:'center',color:recExpanded?IKB:FAINT,border:`1px solid ${recExpanded?IKB:LINE_MED}`,background:'transparent'}}><Play className="w-2.5 h-2.5" strokeWidth={1.25} fill="currentColor"/></button>}
        <Tooltip shortcut="D" label="Tuning"><button onClick={()=>setDroneExpanded(x=>!x)} style={{height:32,padding:'0 12px',display:'inline-flex',alignItems:'center',gap:8,border:`1px solid ${drone.running?IKB:LINE_MED}`,color:drone.running?IKB:MUTED,background:drone.running?IKB_SOFT:'transparent'}}><Waves className="w-3 h-3" strokeWidth={1.25}/><span className="uppercase tabular-nums" style={{fontSize:'10px',letterSpacing:'0.22em',display:'inline-block',minWidth:36,textAlign:'left'}}>{drone.running?`${drone.note}${drone.octave}`:'Tuning'}</span></button></Tooltip>
      </div>
