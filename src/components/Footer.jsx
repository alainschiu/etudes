...
function DronePanel({drone,setDrone,toggleDrone,setDroneExpanded}){
  ...
  return (<div className="px-10 py-5" style={{borderBottom:`1px solid ${LINE}`,background:SURFACE}}>
    ...
    <div className="flex items-baseline justify-between mb-5">
      <div>
        ...
        <div className="flex items-baseline gap-3 mt-0.5">
          ...
          <span className="font-mono tabular-nums" style={{color:FAINT,fontSize:'11px',fontFamily:sans}}>{hz} Hz</span>
          {centsStr&&<span className="font-mono tabular-nums" style={{color:centsColor,fontSize:'11px',fontWeight:500}}>{centsStr}</span>}
        </div>
      </div>
      <div className="flex items-center gap-3">
        <button onClick={toggleDrone} className="w-9 h-9 flex items-center justify-center shrink-0" style={{background:drone.running?IKB:'transparent',color:TEXT,border:`1px solid ${drone.running?IKB:LINE_STR}`,boxShadow:drone.running?`0 0 15px ${IKB}60`:'none'}}>{drone.running?<Pause className="w-3.5 h-3.5" strokeWidth={1.25} fill="currentColor"/>:<Play className="w-3.5 h-3.5 ml-0.5" strokeWidth={1.25} fill="currentColor"/>}</button>
    ...
    <div className="flex items-center gap-3">
      ...
      <span className="font-mono tabular-nums text-xs w-10 text-right" style={{color:MUTED,fontWeight:300}}>{Math.round(drone.volume*100)}</span>
    </div>
  ...
  return (<footer className="shrink-0" style={{borderTop:`1px solid ${LINE_MED}`,background:BG}}>
    ...
    {metroExpanded&&(<div className="px-10 py-6" style={{borderBottom:`1px solid ${LINE}`,background:SURFACE}}>
      ...
      <div className="flex items-center gap-8 flex-wrap pb-4 mb-4" style={{borderBottom:`1px solid ${LINE}`}}>
        ...
        <button onClick={handleTap} className="uppercase px-3 py-1.5 shrink-0" style={{background:IKB,color:TEXT,boxShadow:`0 0 10px ${IKB}40`,fontSize:'10px',letterSpacing:'0.22em'}}>Tap tempo</button>
    ...
    <div className="h-16 flex items-stretch px-10 gap-6">
      ...
      <div className="flex items-stretch flex-1 min-w-0 gap-0" style={{paddingTop:'10px',paddingBottom:'8px'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',justifyContent:'space-between',minWidth:'80px',paddingRight:'16px'}}>
          ...
          <span className="font-mono tabular-nums" style={{fontWeight:300,fontSize:'16px',lineHeight:1,fontVariantNumeric:'tabular-nums lining-nums'}}>{fmtMin(effectiveTotalToday)}{warmupTimeToday>0&&<span style={{color:WARM,fontSize:'9px',marginLeft:'4px',lineHeight:1}}>+◔{Math.floor(warmupTimeToday/60)}</span>}</span>
        </div>
        {/* Divider */}
        ...
        <div style={{display:'flex',flexDirection:'column',justifyContent:'space-between',minWidth:'64px',paddingRight:'16px'}}>
          ...
          <span className="font-mono tabular-nums" style={{fontWeight:300,fontSize:'16px',lineHeight:1,fontVariantNumeric:'tabular-nums lining-nums'}}>{fmtMin(activeItem?sectionTimes[activeItem.type]:0)}</span>
        </div>
        {/* Divider */}
        <div className="w-px self-stretch shrink-0" style={{background:LINE_MED,marginRight:'16px'}}/>
        ...
        <div className="flex-1 min-w-0" style={{display:'flex',flexDirection:'column',justifyContent:'space-between'}}>
          <div className="uppercase truncate flex items-center gap-1.5" style={{color:activeItem?MUTED:(isResting?MUTED:FAINT),fontSize:'9px',letterSpacing:'0.28em',lineHeight:1}}>
            <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${(activeItemId||isResting)?'animate-pulse':''}`} style={{background:activeItemId?IKB:(isResting?MUTED:DIM),boxShadow:activeItemId?`0 0 6px ${IKB}`:'none'}}/>
          ...
          <div className="flex items-baseline gap-4">
            <span className="font-mono tabular-nums tracking-tight" style={{color:statusColor,fontWeight:300,fontSize:'24px',lineHeight:1,textShadow:statusGlow,fontVariantNumeric:'tabular-nums lining-nums'}}>{fmt(statusSec)}</span>
      ...
      <div className="flex items-center gap-3 shrink-0">
        <button onClick={toggleRest} disabled={dayClosed&&!isResting} className="flex items-center gap-2 px-3 py-1.5" style={{border:`1px solid ${isResting?IKB:'transparent'}`,color:isResting?IKB:((dayClosed&&!isResting)?FAINT:MUTED),background:isResting?IKB_SOFT:'transparent',boxShadow:isResting?`0 0 12px ${IKB}40`:'none',cursor:(dayClosed&&!isResting)?'not-allowed':'pointer'}}><Coffee className="w-3 h-3" strokeWidth={1.25}/><span className="uppercase" style={{fontSize:'10px',letterSpacing:'0.22em'}}>{isResting?`${Math.floor(restToday/60)}′ rest`:'Rest'}</span></button>
        <button onClick={isRecording?stopRecording:startRecording} disabled={dayClosed&&!isRecording} className="flex items-center gap-2 px-3 py-1.5" style={{border:`1px solid ${isRecording?IKB:'transparent'}`,color:isRecording?IKB:((dayClosed&&!isRecording)?FAINT:MUTED),background:isRecording?IKB_SOFT:'transparent',boxShadow:isRecording?`0 0 12px ${IKB}50`:'none',cursor:(dayClosed&&!isRecording)?'not-allowed':'pointer'}}>{isRecording?<><Square className="w-3 h-3" strokeWidth={1.25} fill="currentColor"/><span className="uppercase animate-pulse" style={{fontSize:'10px',letterSpacing:'0.22em'}}>REC</span></>:<><Mic className="w-3 h-3" strokeWidth={1.25}/><span className="uppercase" style={{fontSize:'10px',letterSpacing:'0.22em'}}>Record</span></>}</button>
        <button onClick={()=>setDroneExpanded(x=>!x)} className="flex items-center gap-2 px-3 py-1.5" style={{border:`1px solid ${drone.running?IKB:'transparent'}`,color:drone.running?IKB:MUTED,background:drone.running?IKB_SOFT:'transparent',boxShadow:drone.running?`0 0 12px ${IKB}50`:'none'}} title="Tuning pitch (D)"><Waves className="w-3 h-3" strokeWidth={1.25}/><span className="uppercase" style={{fontSize:'10px',letterSpacing:'0.22em'}}>{drone.running?`${drone.note}${drone.octave}`:'Tuning'}</span></button>
      </div>
      ...
      <div className="flex items-center gap-4 shrink-0 pl-6" style={{borderLeft:`1px solid ${LINE_MED}`}}>
        ...
        <button onClick={()=>setMetronome(m=>({...m,running:!m.running}))} className="w-9 h-9 flex items-center justify-center shrink-0" style={{background:metronome.running?IKB:'transparent',color:TEXT,border:`1px solid ${metronome.running?IKB:LINE_STR}`,boxShadow:metronome.running?`0 0 15px ${IKB}60`:'none'}}><MetronomeIcon size={14}/></button>
        <div className="flex gap-1.5 items-end" style={{height:'22px'}}>
          {Array.from({length:metronome.beats}).map((_,i)=>(<div key={i} className="flex gap-px items-end" style={{height:'100%'}}>{Array.from({length:isDotSub?1:metronome.subdivision}).map((_,si)=>{const isA=metronome.running&&currentBeat===i&&currentSub===si;const isM=si===0;return <div key={si} style={{alignSelf:'flex-end',height:isA?(i===0&&isM?'22px':isM?'16px':'10px'):(isM?'12px':'6px'),width:isA?'2px':'1px',background:isA?IKB:(isM?DIM:'rgba(232,223,200,0.1)'),boxShadow:isA?`0 0 6px ${IKB}`:'none',transition:'height 75ms, width 75ms'}}/>;})}</div>))}
        </div>
        <div className="flex items-center gap-2 shrink-0"><input type="range" min="40" max="240" value={metronome.bpm} onChange={e=>setMetronome(m=>({...m,bpm:+e.target.value}))} className="w-24" style={{accentColor:IKB}}/><div className="font-mono text-sm tabular-nums w-8 text-right" style={{fontWeight:300}}>{metronome.bpm}</div><div className="uppercase" style={{color:FAINT,fontSize:'9px',letterSpacing:'0.22em'}}>bpm</div></div>
