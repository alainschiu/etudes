...
import {SettingsModal,HelpModal,ConfirmModal,PromptModal} from './components/modals.jsx';
    ...
    <div className="h-screen flex flex-col" style={{background:BG,color:TEXT,fontFamily:sans}}>
      ...
      <div className="flex-1 flex overflow-hidden">
        <main className="flex-1 overflow-auto etudes-scroll">
          {view==='today'&&<TodayView {...{...commonProps,todaySessions,setTodaySessions,moveSession,hideSession,addSessionType,toggleSessionWarmup,removeItemFromSession,addItemToSession,setSessionTarget,setItemTarget,routines,loadedRoutine,loadRoutine,resetToFree,saveRoutine,sectionTimes,activeItemId,activeSpotId,activeSessionId,expandedItemId,setExpandedItemId,startItem,stopItem,updateItem,deleteItem,addItem,workingOn,toggleWorking,setPdfDrawerItemId,dailyReflection,setDailyReflection,totalToday,effectiveTotalToday,warmupTimeToday,restToday,setPromptModal,dragIdx,dragOverIdx,handleDragStart,handleDragOver,handleDrop,handleDragEnd,deleteRecording,sessionRefs,reflectionRef,endDay,dayClosed,reopenDay,editingTimeItemId,setEditingTimeItemId,editItemTime,editSpotTime,addSpot,updateSpot,deleteSpot}}/>}
          ...
          {view==='repertoire'&&<RepertoireView {...{...commonProps,setItems:s.setItems,updateItem,deleteItem,setPdfDrawerItemId,activeItemId,activeSpotId,startItem,stopItem,addItem,dayClosed,addSpot,updateSpot,deleteSpot,moveSpot,editSpotTime,addPerformance,updatePerformance,deletePerformance}}/>}
      ...
      {showSettings&&<SettingsModal settings={settings} setSettings={setSettings} storageMode={storageMode} onExportMd={()=>exportLog('md')} onExportTxt={()=>exportLog('txt')} onExportJson={exportJson} onImportClick={()=>importInputRef.current?.click()} onClose={()=>setShowSettings(false)}/>}
      {showHelp&&<HelpModal onClose={()=>setShowHelp(false)}/>}
      {pdfItem&&<PdfDrawer {...{pdfItem,items,pdfUrlMap,itemTimes,activeItemId,activeSpotId,startItem,stopItem,updateItem,addPdfToItem,removePdfFromItem,renamePdf,setDefaultPdf,fmt,setPromptModal,setConfirmModal,onClose:()=>setPdfDrawerItemId(null),dayClosed,addSpot,updateSpot,deleteSpot,editSpotTime}}/>}
