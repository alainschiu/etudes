export function openDB(){return new Promise((res,rej)=>{try{const r=indexedDB.open('etudes',2);r.onupgradeneeded=()=>{const db=r.result;if(!db.objectStoreNames.contains('pdfs'))db.createObjectStore('pdfs');if(!db.objectStoreNames.contains('recordings'))db.createObjectStore('recordings');if(!db.objectStoreNames.contains('pieceRecordings'))db.createObjectStore('pieceRecordings');};r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error);}catch(e){rej(e);}});}
